...
| 分区名 | 起始地址 | 大小 | 作用 |
| --- | --- | --- | --- |
| Bootloader | `0x08000000` | 64KB | 上电第一行代码，负责验签、搬运固件、跳转 |
| App 主区 | `0x08010000` | 448KB | 当前运行的应用程序 (APP) |
| 下载暂存区 | `0x08080000` | 448KB | 接收 OTA 新固件，差分还原也在这里 |
| 参数区 | `0x080F0000` | 64KB | 存升级状态、版本号、断点续传偏移量等 |
